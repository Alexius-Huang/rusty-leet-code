...
pub fn run(nums: Vec<i32>) -> i32 {
    ...
    match len {
        0 => {
            panic!("Expect to have at least 1 element");
        }
        1 => {
            return nums[0];
        }
        2 => {
            return if nums[0] > nums[1] { nums[1] } else { nums[0] };
        }
        _ => (),
    }
    ...
    loop {
        let left_slice_edge = (nums[lower_bound], nums[mid_index - 1]);
        let is_left_ascending = left_slice_edge.1 - left_slice_edge.0 >= 0;

        let right_slice_edge = (nums[mid_index], nums[upper_bound - 1]);
        let is_right_ascending = right_slice_edge.1 - right_slice_edge.0 >= 0;
